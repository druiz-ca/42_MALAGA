...
/*   Updated: 2025/02/15 13:43:48 by druiz-ca         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
    // tenog que hacer una cad temp pq sino modifico el nombre guardado y solo
    //necesito recortar el nombre que se mostrara en la tabla no el original!!
    std::string temp = str; 
    std::cout << " | ";
    if (temp.size() > 10)
    {
        temp.resize(9); // sirve para cortar la cadena a 9 caracteres
        std::cout << temp;
        std::cout << ".";
    }
    else // por no tener el else me imprimia dos veces el nombre!!!
        std::cout << std::setw(10) << temp; // sirve para darle un ancho de 10 caracteres
}
...
int main(void)
{
    ...
    while (1)
    {
        ...
        else if (command == "SEARCH" && PB.num > 0) // sino contemplo que no haya entradas se queda pillado!
        {
