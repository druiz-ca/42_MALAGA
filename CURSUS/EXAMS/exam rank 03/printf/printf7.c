...
// por no recibir num como long no imprimia nada en el caso del "x"
int ft_putnbr(long num, int base, int cont)
{
    ...
    i++;
    // me olvidad de guardar en cont el valor devuelto x putstr!!!!
    cont = ft_putstr(res, i, cont);
    return (cont);
}
...
int ft_printf(char *cad, ...)
{
    ...
    while (cad[i])
    {
        ...
        {
            cont = ft_putnbr(va_arg(args, int), 10, cont);
            i++;
        }
        else if (cad[i] == '%' && cad[i + 1] == 'x')
        {
            cont = ft_putnbr(va_arg(args, int), 16, cont);
            i++;
        }
        else
        {
